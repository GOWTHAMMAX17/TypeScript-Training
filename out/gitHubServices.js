...
var gitHubServices = /** @class */ (function () {
    ...
    gitHubServices.prototype.getUserInfo = function (userName, cb) {
        request.get('https://api.github.com/users/' + userName, OPTIONS, function (error, response, body) {
            var user = new User_1.User(body);
            cb(user);
        });
    };
    gitHubServices.prototype.getRepos = function (userName, cb) {
        request.get('https://api.github.com/users/' + userName + '/repos', OPTIONS, function (error, response, body) {
            var repoArray = body;
            var repos = repoArray.map(function (repo) { return new Repos_1.Repos(repo); });
            cb(repos);
        });
    };
